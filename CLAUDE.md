...
scripts/
  prefetch-electron.mjs     # Fetch electron binary + repair macOS framework symlink (pretest:e2e)
...
- **Git hooks (husky):** `.husky/pre-commit` runs `lint + unit + build` for fast feedback on every commit. There is no pre-push hook — E2E runs in CI instead (`e2e` job in `.github/workflows/ci.yml`), because Playwright + Electron is slow (~25–45s) and occasionally flakes, and a flake at pre-push blocks the developer while a flake in CI is one re-run and stays visible. That job is a `macos-latest` / `windows-latest` / `ubuntu-latest` matrix, so every shipped platform is exercised; Linux runs under `xvfb` (the other two have a real display) and installs Chromium's shared libraries first. `npm run test:e2e` is preceded by the `pretest:e2e` hook (`scripts/prefetch-electron.mjs`), which fetches Electron's lazily-downloaded binary once up front — two Playwright workers racing that download exec a half-written binary — and repairs the framework symlink that macOS extracts sometimes drop. The trade-off: a branch with failing E2E can be pushed, which costs a red check on the PR rather than a blocked push.
...
- **Release pipeline:** Two-step Release-PR flow driven by release-please (`release-please-config.json`, `.release-please-manifest.json`), split across two workflows along the action's own `skip-*` inputs. **Step one**, `.github/workflows/release-pr.yml`, is `workflow_dispatch` only and runs with `skip-github-release: true`: it opens or refreshes a `chore(main): release X.Y.Z` PR carrying the `package.json` bump and the `CHANGELOG.md` entry, and takes an optional `release-as` input to force a version. It is deliberately not on `push` — a standing, self-reopening release PR is noise at this size, and the dispatch is the explicit "start a release" signal `npm run release` used to be. The catch: commits landing after the PR opens are not picked up (version and changelog freeze at dispatch), so re-dispatch to refresh before merging. **Step two**, `.github/workflows/release.yml`, runs on every push to `main` with `skip-github-pull-request: true` and is a no-op unless the pushed commit is a merged release PR; this half cannot be manual, because release-please only recognises a merged release PR while looking at a push to `main`. **Merging the PR is the release decision** — there is no separate approval. The bump level comes from Conventional Commit prefixes since the last tag — `feat!`/`fix!`/`BREAKING CHANGE:` → major, `feat:` → minor, everything else → patch. On merge, one run does everything: `release-please` tags the merge commit and creates the GitHub Release as a **draft**, the three `build` jobs package unsigned artifacts on `macos-latest` / `windows-latest` / `ubuntu-latest` (`npm run dist` with `CSC_IDENTITY_AUTO_DISCOVERY=false`, checked out at the new tag rather than at `main`), `publish` attaches them with `gh release upload` and then flips the draft, and `homebrew-bump` opens a cask PR on the tap. The draft step exists so a release is never visible without its binaries — the tag lands minutes before the builds finish. Build and publish stay in one workflow deliberately: a tag pushed with `GITHUB_TOKEN` does **not** fire `on: push: tags`, so a handoff would need a PAT. The release PR is an ordinary PR, so `ci.yml` (lint, coverage, build, E2E on all three platforms) gates it before the merge — this replaces the guards the old local `scripts/release.mjs` performed. Node pinned via `.nvmrc`; `package.json` version is the source of truth, stamped into artifact filenames by electron-builder, and release-please keeps it and the tag in sync. Signing not configured.
